...
- **Researches instead of remembering.** Three research subagents per run: current AI-design tells, platform guidelines with numbers, and the stack landscape with real versions. Real competitors are captured headlessly and become the bar the critic grades against. Findings are cached per category and platform.
